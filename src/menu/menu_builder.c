...
// Its own text when it has one, otherwise the translated message.
static char* checkboxMenuItemText(struct MenuBuilderElement* element) {
    char* message = element->params->params.checkbox.message;
    return message ? message : translationsGet(element->params->params.checkbox.messageId);
}
